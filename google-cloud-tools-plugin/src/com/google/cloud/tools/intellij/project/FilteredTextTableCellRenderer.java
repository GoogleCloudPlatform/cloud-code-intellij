...
public class FilteredTextTableCellRenderer extends DefaultTableCellRenderer {
  ...
  String highlightFilterText(String filterText, String text) {
    if (!Strings.isNullOrEmpty(filterText) && !Strings.isNullOrEmpty(text)) {
      int index = text.toLowerCase().indexOf(filterText.toLowerCase());
      if (index >= 0) {
        String firstPart = text.substring(0, index);
        String boldPart = text.substring(index, index + filterText.length());
        String lastPart = text.substring(firstPart.length() + boldPart.length());
        return "<html>" + firstPart + "<b>" + boldPart + "</b>" + lastPart;
      }
    }
    return text;
  }
