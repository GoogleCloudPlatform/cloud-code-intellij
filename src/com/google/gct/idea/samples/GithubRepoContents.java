...
public final class GithubRepoContents {
   ...
   */
  @NotNull
  public static GithubRepoContents download(@NotNull Project project,
                                            @NotNull String url,
    ...
    if (!unzippedDir.exists() || unzippedDir.lastModified() == 0 ||
      ...
      try {
        Outcome<File> outcome =
          DownloadUtil.provideDataWithProgressSynchronously(project, "Downloading project from GitHub", "Downloading zip archive" +
                                                                                                        DownloadUtil.CONTENT_LENGTH_TEMPLATE +
                                                                                                        " ...", new Callable<File>() {
            @Override
  ...
  /**
   * Use {@link #download(Project, String, String, File)} instead.
   */
  private GithubRepoContents() {
  }

  /**
   * @return any error message encountered during {@link #download(Project, String, String, File)}. This will be {@code null} if no error
